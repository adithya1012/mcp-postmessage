// Patient Dashboard Data Management Functions
...
let patientData = {
  ...
  medications: [
    { name: 'Lisinopril', dosage: '10mg', frequency: 'Daily', prescribedDate: 'Jan 1, 2025' },
    { name: 'Metformin', dosage: '500mg', frequency: 'Twice daily', prescribedDate: 'Dec 15, 2024' }
  ]
};
...
function updateBloodSugar(value) {
  ...
  if (sugarElement) {
    sugarElement.textContent = value;
...
function updateNextAppointment(date) {
...
function updateAllergies(allergies) {
  ...
  if (allergyItemsElement && allergyCountElement) {
    // Update allergy list
    const allergyList = allergies.map(allergy => 
      `• ${allergy.name} (${allergy.severity})`
    ).join('<br>');
    allergyItemsElement.innerHTML = allergyList;
    
    // Update count
    allergyCountElement.textContent = allergies.length;
...
function updateMedicationsDisplay() {
  ...
  if (medicationsListElement && patientData.medications.length > 0) {
    const medicationsList = patientData.medications.map(med => 
...
function addMedicalHistoryEntry(icon, title, time) {
...
function updatePatientInfo(name, patientId, dob, age) {
...
function updatePatientDashboard(newPatientData) {
  if (newPatientData.name) {
    updatePatientInfo(newPatientData.name, newPatientData.patientId, newPatientData.dob, newPatientData.age);
  }
  
  if (newPatientData.bloodPressure) {
    updateBloodPressure(newPatientData.bloodPressure.systolic, newPatientData.bloodPressure.diastolic);
  }
  
  if (newPatientData.bloodSugar) {
...
function simulateBloodPressureUpdate() {
...
function simulateBloodSugarUpdate() {
  const sugar = Math.floor(Math.random() * 50) + 80; // 80-130
  updateBloodSugar(sugar);
}

function simulateVisitUpdate() {
  const currentVisits = parseInt(document.getElementById('total-visits').textContent);
  updateTotalVisits(currentVisits + 1);
  updateLastVisit('Today');
  addMedicalHistoryEntry('🏥', 'Regular Checkup', 'Today');
}

function simulateAddMedication() {
  const medications = [
    { name: 'Aspirin', dosage: '81mg', frequency: 'Daily', prescribedDate: 'Today' },
    { name: 'Vitamin D', dosage: '1000IU', frequency: 'Daily', prescribedDate: 'Today' },
    { name: 'Omega-3', dosage: '1000mg', frequency: 'Twice daily', prescribedDate: 'Today' }
  ];
...
function initializeDashboard() {
  ...
  console.log('- patientDashboard.updateBloodPressure(systolic, diastolic)');
  console.log('- patientDashboard.updateBloodSugar(value)');
  console.log('- patientDashboard.updateLastVisit(date)');
  console.log('- patientDashboard.updateTotalVisits(count)');
  console.log('- patientDashboard.updateNextAppointment(date)');
  console.log('- patientDashboard.updateAllergies([{name, severity}])');
  console.log('- patientDashboard.addMedication({name, dosage, frequency, prescribedDate})');
  console.log('- patientDashboard.addMedicalHistoryEntry(icon, title, time)');
  console.log('- patientDashboard.updatePatientInfo(name, id, dob, age)');
  console.log('- patientDashboard.updatePatientDashboard(patientData)');
  console.log('- patientDashboard.getContext() - Returns complete patient data');
  console.log('- patientDashboard.toggleMedications() - Toggle medications section');
  console.log('\nTest functions:');
